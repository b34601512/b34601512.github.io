...
- 首页演示：切团队/个人/离线话术（页签蓝线跟动）、点套号与一级分类切换、二级分类展开收起、搜索（结果带路径、无结果提示、Esc 退出）、单击选中、双击把话术贴进聊天输入框（不直接发送）、点话术行左侧纸飞机直接发送并出现绿色气泡与客户回复、常用短语直接发送、聊天窗口发送按钮能发出输入框里的内容、重置后回到初始状态。
