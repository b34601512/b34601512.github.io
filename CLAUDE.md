...
首页 H2 落点：看一眼就会用（试用演示）/ 真实界面 / 功能 / 常见问题——**不重复写 why 页的三条痛点**，只在功能小节末尾放一条指向 `why.html` 的文字链。
...
- 内链：首页功能小节末尾 → `why.html`；`why.html` 页尾 → 首页演示、顶部按钮 → 下载/定价；`pricing.html` → `why.html`；页脚与导航由 `sitePages` 自动带上新页面。

### 一个信息只说一次（内容归属）

用户明确要求过：同样的内容不许在两个地方各写一遍。每条信息只有一个“主”：

| 内容 | 唯一归属 | 其他页面怎么写 |
| --- | --- | --- |
| 产品一句话定义、怎么用（双击贴入 / 回车发送 / 箭头发送） | `index.html` 首屏 lead + 演示区 | 别处不重述机制；`why.html` 只链到演示 |
| 能力清单（三套话术库、Alt+Q/W/R、Excel 导入、备份） | `index.html` 功能 | 别处不重复罗列 |
| 三个真实问题 + 十套模块解法 + 与平台快捷短语对比 | `why.html` | 首页只放一条文字链 |
| 价格、云端权益 | `pricing.html` | 别处不写价格数字 |
| 联系方式 | `contact.html` | 别处不写微信号/邮箱 |
| 平台兼容、三种话术域定义、内容范围、搬家做法 | 首页 FAQ | 不与功能卡/why 页重叠 |

`check-site.mjs` 内置了这条规则的自动检查：同一句话（≥ 12 字）出现在两个页面的正文里、或同一页里写两遍，构建自检会直接报错。写文案前先看这张表。
...
- SEO：每页恰一个 H1；title/description 四页互不重复且含各自主关键词；`meta keywords`、canonical、og:image（指向 `assets/og.png`）齐备；JSON-LD 能被 `JSON.parse` 解析且没有 `aggregateRating`；首页 FAQ 条数与 JSON-LD `FAQPage` 一致；`sitemap.xml` 含四个页面；内链：首页功能小节 → `why.html`、`why.html` → 首页演示/定价/下载、`pricing.html` → `why.html`；`check-site.mjs` 的重复内容检查无报错。
- `why.html`：正文保持简短（约 400 字内），只有两个 H2、对比表 2 行、页尾一条演示链接。
