...
- **数字键顶部与小键盘都要认**（客户端用 `QKeySequence("0".."9")`，两种键盘都匹配）：切套快捷键和数字发送都走同一个 `readDigit()`，别只判 `event.code === "DigitN"`，否则小键盘无效。
