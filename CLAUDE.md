...
- 首页首屏下方直接是可交互演示（不再有「三步用起来」小节），路径是「先看它怎么用，再看真实界面」；加内容优先往真实界面/功能里加，不要再往首屏下塞过渡性小节。
...
- 演示操作路径：页签切话术域、点套号整套切换、点一级分类切分类、二级分类展开收起、单击选中、双击贴话术到输入框、点行左侧纸飞机直接发送（粘贴 + 回车，与客户端 `list_box.py` 的 `insert_dbl`/`send` 一致）、聊天窗口自己的发送按钮发出、常用短语直发、搜索（全部范围，命中结果带「话术域 · 套号 套名 › 一级分类 › 二级分类」路径）、**Alt+Q 定位搜索框（带一次短高亮）**、客户自动回复、重置。
...
首页 H2 落点：可交互界面实操试用 / 真实界面 / 功能 / 常见问题。
...
- `screenshot.webp`：940×699，质量 82，143 KB → 42.5 KB；源图 `assets/screenshot2.png` 与产物一起提交，改图时重新跑一次函数即可（`build_screenshot`）。
